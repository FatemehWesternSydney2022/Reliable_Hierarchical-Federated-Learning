      ...
      for backup_server in self.backup_servers:
          if not backup_server.is_failed():
              ...
              with open(log_file_path, "a") as log_file:
                  log_file.write(f"{self.server_id},FAILED,{backup_server.server_id}\n")
              return
...
def simulate_failures(args, unavailability_tracker, failure_log, round_number, training_times, selected_clients, lstm_model, failure_history, failure_events):
    """Simulate client failures based on real trace events."""
    ...
    # ✅ Step 2: Check which clients should fail according to the real trace
    round_duration_sec = args.get('ROUND_DURATION_SEC', 10)  # e.g., 10 seconds per round
    current_time = round_number * round_duration_sec

    newly_failed_clients = []
    for client_id in available_clients:
        events = failure_events.get(client_id, [])
        for fail_time, recover_time in events:
            if fail_time <= current_time <= recover_time:
                newly_failed_clients.append(client_id)
                break

    # ✅ Step 3: Update failure log
    for client_id in newly_failed_clients:
        failure_duration = next(
            (recover_time - fail_time for fail_time, recover_time in failure_events[client_id] if fail_time <= current_time <= recover_time),
            args['FAILURE_DURATION']  # fallback
        )

        avg_training_time = np.mean([training_times.get(cid, 0) for cid in selected_clients]) if selected_clients else 0
        recovery_time_remaining = failure_duration - avg_training_time

        unavailability_tracker[client_id] = 1  # Mark as unavailable
        failure_log.append([client_id, failure_duration, recovery_time_remaining])

        # Update failure history
        current_timestamp = time.time()
        if client_id not in failure_history:
            failure_history[client_id] = []
        failure_history[client_id].append(current_timestamp)
    ...
    # ✅ Step 4: Update recovery times for ongoing failures
    avg_training_time = np.mean(list(training_times.values())) if training_times else 0

    for client in failure_log:
        client[2] -= avg_training_time
        client[2] = max(0, client[2])
        if client[2] == 0:
            unavailability_tracker[client[0]] = 0
            recovered_this_round.add(client[0])

    # ✅ Step 5: Recover clients
...
def HierFL(args, trainloaders, valloaders, testloader):
    ...
    # ✅ Load the real failure trace
    def load_failure_trace(file_path):
      """Load failure trace and prepare client-specific failure schedules."""
      df = pd.read_csv(file_path)

      # Expect columns: client_id, fail_time (seconds), recover_time (seconds)
      failure_events = {}

      for _, row in df.iterrows():
          client_id = int(row["node_id"])
          fail_time = float(row["event_start_time"])
          recover_time = float(row["event_stop_time"])

          if client_id not in failure_events:
              failure_events[client_id] = []
          failure_events[client_id].append((fail_time, recover_time))

      return failure_events


    # ✅ Load failure trace
    failure_trace_path = "event_trace.csv"
    failure_events = load_failure_trace(failure_trace_path)
    ...
    for round_number in range(1, args['GLOBAL_ROUNDS'] + 1):
        ...
        failure_log, recovered_this_round= simulate_failures(args, unavailability_tracker, failure_log, round_number, training_times, selected_clients, lstm_model, failure_history, failure_events)
...
def main():
    ...
    def load_datasets(num_clients: int, batch_size: int = 20):
